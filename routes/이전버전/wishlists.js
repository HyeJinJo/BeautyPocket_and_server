...
router.post('/add',  function(req, res, next) {
    ...
    pool.getConnection(function(error, connection) {
        if (error) {
            ...
            if (req.file) {
                sql = 'insert into WishList(user_id, brand_id, pro_id, wish_title, wish_price, wish_image, wish_memo) values(?,?,-1,?,?,?,?)';
                value = [req.body.user_id, req.body.brand_id, req.body.wish_title, req.body.wish_price, req.file.location, req.body.wish_memo];
                ...
                connection.query(query, [req.body.pro_id], function(error1, rows1) {
                    if (error1) {
                        ...
                        var data = rows1[0];
                        var iQuery = 'insert into WishList(user_id, brand_id, pro_id,wish_title, wish_price, wish_image, wish_memo ) values(?,?,?,?,?,?,?)';
                        var iValue = [req.body.user_id, req.body.brand_id, req.body.pro_id, data.pro_name, data.pro_price, data.pro_image, req.body.wish_memo];
...
router.get('/all/:user_id', function(req, res, next) {
    pool.getConnection(function(error, connection) {
        if (error) {
            ...
            var brand_id = 0;
            var value = [req.params.user_id, brand_id];
